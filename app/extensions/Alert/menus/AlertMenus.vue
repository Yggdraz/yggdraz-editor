...
<template>
  <BubbleMenu :editor="editor" :shouldShow="shouldShow" :tippy-options="{
    ...
    plugins: [sticky],
    sticky: 'popper',
  }" :updateDelay="0" pluginKey="alert">
    <div class="bg-white rounded-lg dark:bg-neutral-800 shadow-sm border border-neutral-200 dark:border-neutral-700">
      <div class="flex gap-1 items-center">
        <ActionButton :is-active="() => editor.isActive('alert', { type: 'note' })" :tooltip="$t('editor.alert.note')"
          icon="i-lucide-sticky-note" :title="$t('editor.alert.note')"
          @click="editor.chain().focus().setAlertType('note').run()" />
        <ActionButton :is-active="() => editor.isActive('alert', { type: 'info' })" :tooltip="$t('editor.alert.info')"
          icon="i-lucide-info" :title="$t('editor.alert.info')"
          @click="editor.chain().focus().setAlertType('info').run()" />
        <ActionButton :is-active="() => editor.isActive('alert', { type: 'warning' })"
          :tooltip="$t('editor.alert.warning')" icon="i-lucide-triangle-alert" :title="$t('editor.alert.warning')"
          @click="editor.chain().focus().setAlertType('warning').run()" />
        <ActionButton :is-active="() => editor.isActive('alert', { type: 'danger' })"
          :tooltip="$t('editor.alert.danger')" icon="i-lucide-circle-x" :title="$t('editor.alert.danger')"
          @click="editor.chain().focus().setAlertType('danger').run()" />
        <ActionButton :is-active="() => editor.isActive('alert', { type: 'success' })"
