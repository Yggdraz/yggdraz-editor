...
<template>
  <div
    class="p-1 shadow-md rounded-md border border-neutral-200 bg-white dark:bg-neutral-800 dark:border-neutral-700 max-w-fit">
    <div v-for="item in items" :key="item.label" class="relative" @click="handleClick(item)"
      ...
      <div v-if="item.children && hoveredState[item.label]"
        ...
        <UButton v-for="subItem in item.children" :key="subItem.label" :label="subItem.label" color="neutral" size="xs"
          variant="ghost" @click="handleClick(subItem)" />
      </div>
    </div>
  </div>
</template>
